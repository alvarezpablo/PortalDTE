...
//exit();
	...
	function renderGpuertoPageStart($eyebrow, $title, $meta, $panelTitle, $panelSubtitle, $chipText, $chipIcon = 'bi bi-box-seam') {
		echo <<<HTML
<!doctype html>
<html lang="es">
<head>
...
if($fileError==0 && $file != "") {
		renderGpuertoPageStart(
			'Carga masiva Laudus',
			'Grupo Puerto',
			'Se conserva intacto el flujo legacy de lectura del Excel, generacion DTE, integracion con Laudus y registro en OpenDTE.',
			'<i class="bi bi-file-earmark-spreadsheet me-2"></i>Resultado del procesamiento',
			'Se muestran las respuestas actuales del proceso junto con el PDF y la respuesta Laudus del flujo existente.',
...
//	date_default_timezone_set('PRC');
	...
	try {
		...
		$objPHPExcel = $objReader->load($inputFileName);
			...
			echo "<div class='form-actions'><a href='gpuerto.php' class='btn btn-outline-secondary'><i class='bi bi-arrow-left-circle me-2'></i>Volver</a></div>";
			renderGpuertoPageEnd();
		exit;
	}
	...
	$iniDTEAnt = "";
		...
		echo "<div class='panel-note'><strong>Iniciando proceso del archivo.</strong> El modulo mantiene el procesamiento original del Excel, la emision DTE y la integracion con Laudus sin alterar contratos ni endpoints.</div>";
		echo "<div class='results-wrap'><table class='results-table'><tr><th>Respuesta</th><th>PDF</th><th>Resp. Laudus</th></tr>";
...
//						exit;
						...
						if($sResp["estado"] == 1){
							...
							echo "<tr><td bgcolor='#FCA7B2' colspan='3'>" . $sResp["glosa"] . "</td></tr>";
							//$conn->failTrans();	// rollback
						}
					}
					else
...
//			exit;
			...
			if(!$result->EOF){
				...
				$emailCliente = substr($emailCliente,0,40);
...
//$jsonString  2022-02-28T20:32:51-03:00
				...
				$objCli = idCliLaudus($rutCliente);    // 				return array('estado' => "0", "glosa" => "Cliente no Existe");
				$idCliLau = $objCli["id"]; 
				$cliExis = $objCli["estado"]; 
//				print_r($objCli);
...
/*				$jsonEnc = "{
				  ...
				  \"docNumber\": $folioDTE,
				  \"customer\": {
					\"customerId\": " . str_replace("-","",$rutCliente) . ",
					\"name\": \"$nomCliente\",
					\"legalName\": \"$nomCliente\",
					\"email\": \"$emailCliente\"
				  },   */
				$jsonEnc = "{
				  \"salesInvoiceId\": \"\",
...
/*				  \"totalsOriginalCurrency\": {
				 ...
				 $jsonEnc .= "\"payInOriginalCurrency\": true,
				  \"deliveryCost\": 0,
				...
				$existeEmpresa = false;
				echo "<tr><td bgcolor='#FCA7B2' colspan='3'>Error empresa emisora no registrada en opendte, no se puede procesar rut emisor $rutEmisor y folio " . $folioDTE . "</td></tr>";
			}
		  ...
		  $jsonDet .= "
			{
			  ...
			  \"itemOrder\": 0,
				  \"product\": {
					...
					// \"allowUserChangePrices\": true
		$jsonDet .= "			},
			  \"itemDescription\": \"$descItem\",
			  \"quantity\": $cantItem,
			  \"originalUnitPrice\": $precioItem, ";
		//	  \"parityToMainCurrency\": $precioItem,
		$jsonDet .= "	  \"unitPrice\": $precioItem,
			  \"VAT\": $precioIva,
			  \"VATRate\": 19,
...
/*
		...
		echo "</table></div>";
		echo "<div class='panel-note mt-3 mb-0'><strong>Fin del proceso del archivo.</strong> Puede volver al formulario para ejecutar una nueva carga con el mismo flujo operativo del modulo.</div>";
		echo "<div class='form-actions'><a href='gpuerto.php' class='btn btn-outline-secondary'><i class='bi bi-arrow-left-circle me-2'></i>Cargar otro archivo</a></div>";
		renderGpuertoPageEnd();
...
function firmarDTE_old($xml, $rutEmisor, $dvEmisor, $tipoDTE=""){
...
//$wsdl = file_get_contents($WSDLFirmaDTE."?wsdl");
//echo $wsdl;
//exit;

$wsdlLocal = "/opt/opendte/httpdocs/FirmaDTE.wsdl";
...
//        'location' => $_LINK_BASE_WS . 'OpenDTEWS/services/FirmaDTE.FirmaDTEHttpEndpoint/', // Ubicaci�n  del endpoint
	    'uri' => 'http://ws.opendte.cl',
		'stream_context' => $context,
        'cache_wsdl' => WSDL_CACHE_NONE
    ];
//$soapClient = new SoapClient($WSDLFirmaDTE."?wsdl" , $options);
//$soapClient = new SoapClient($WSDLFirmaDTE."?wsdl");
//$soapClient = new SoapClient($wsdlLocal , $options);
...
/*$_WSDL_ENVIO = "http://10.0.0.210:8080/OpenDTEWS/services/FirmaDTE";
		$soapClient=new SoapClient($_WSDL_ENVIO . "?wsdl");
//		$soapClient=new SoapClient("http://desa.opendte.cl:8080/OpenDTEWS/services/FirmaDTE?wsdl");
		$soapClient->__setLocation($_WSDL_ENVIO);
		$parametros=array();
		$parametros["RUTEmisor"]=$aRut[0]; 
		$parametros["DVEmisor"]=$aRut[1];
		$parametros["tipoArchivo"]="XML";
		$parametros["archivo"]=$xml;
		$parametros["apikey"]="";
		$r = $soapClient->firmaDTE($parametros);
...
//		$result = $client->call('firmaDTE', array('parameters' => $parametros), '', '', false, true);
...
//		$err = $client->getError();
//		if ($err) {
//			return array('estado' => 0, "glosa" => "Error al procesar rut emisor $rutEmpr y folio " . $folio );
//		} else {
//			print_r($result);
...
//					echo "$rutEmpr, $folio, $tipo, $sDestinatario";
					...
					for($i=0; $i < sizeof($aEmail); $i++){
						if(strpos($aEmail[$i],"@") !== false)
							reenviarXML($rutEmpr, $folio, $tipo, "XML", trim($aEmail[$i])); 				
					}
				}

				if($estado == "1" && $cliExis == "1"){		// llamo a laudus
					$ejemplo = new laudusAPI_Ejemplos();
//					print_r($ejemplo);
//					echo "JSON: " . $jsonString . "<br><br>";
					if ($ejemplo->obtenerToken($_USER_LAUDUS_,$_PASS_LAUDUS_,$_RUT_EMP_LAUDUS_)) {
						if ($ejemplo->postFactura($jsonString)) {
							$msgLaudus = "Factura Creada con salesInvoiceId : " . $ejemplo->invoice->salesInvoiceId;
							$estLaudus = "1";
							$estdb = "2";
						}
						else{
						...
						$respLaudus = json_encode($ejemplo->invoice);
					}					
				}

				if($cliExis != "1"){
					$msgLaudus = "Cliente no Existe en Laudus";
					$estLaudus = "0";
					$estdb = "3";
				}
				
				$sql = "update gpuerto_enc set estado='" . str_replace("'","''",$estdb) . "', 
								pdf = '" . str_replace("'","''",$pdf) . "', 
								resp_openb='" . str_replace("'","''",$respOpen) . "', 
								json_laudus = '" . str_replace("'","''",$jsonString) . "',
								resp_laudus = '" . str_replace("'","''",$respLaudus) . "' 
								where 
								tipo_docu = '" . str_replace("'","''",$tipo) . "' AND 
								rut_emisor = '" . str_replace("'","''",$aRut[0]) . "' AND 
								folio_erp = '" . str_replace("'","''",$folio) . "' ";	
				nrExecuta($conn, $sql);

// 							

			if($estado == "1")
				return array('estado' => "1", "glosa" => "Procesado rut emisor $rutEmpr, folio " . $folio . " y tipo $tipo", "pdf" => $pdf, "estadoLaudus" => $estLaudus, "msgLaudus" => $msgLaudus );
			else
...
if($file=="") {
		renderGpuertoPageStart(
			'Carga masiva Laudus',
			'Grupo Puerto',
			'Cargue el archivo Excel usando el mismo endpoint, el mismo nombre de campo y el mismo comportamiento del formulario legacy.',
			'<i class="bi bi-cloud-arrow-up me-2"></i>Subida de archivo',
			'Se conserva el envio hacia gpuerto.php con el campo sFileCaf y el bloqueo del boton al enviar.',
			'Grupo Puerto',
			'bi bi-building'
		);
	?>
	...
	<div class="panel-note">
		Seleccione el archivo de carga y ejecute el proceso con la misma logica actual. Los documentos de apoyo siguen disponibles para revisar el formato esperado.
	</div>

	<form name="_FFORM" enctype="multipart/form-data" action="gpuerto.php" method="post" onsubmit="desac();">
