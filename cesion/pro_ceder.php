<?php
  ...
  if($ok == "OK"){
	  try{
		...
		if(!$result->EOF){
			//header("ceder.php?msg=2");
			header("location:form_ceder.php?msg=2");
			exit;
		}
		...
		if(!$result->EOF){
			...
			$mont_tot_dte = trim($result->fields[0]);
			if($monto_cesion > $mont_tot_dte){
				//header("ceder.php?msg=3");
				header("location:form_ceder.php?msg=3");
				exit;
			}
		}
		...
		//header("ceder.php?msg=0");
		header("location:form_ceder.php?msg=0");
		exit;	  
	  }
  } 
