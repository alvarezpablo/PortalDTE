<?php
	...
	$sLinkActual = "dte/list_dte.php?_EST_DTE=" . $_EST_DTE . "&nTipoDocu=" . $nTipoDocu . "&nAnio=" . $nAnio . "&";
	...
	if(!$resultAnios->EOF){
		$nAnioMenor = trim($resultAnios->fields["anio_menor"]);
		$nAnioMayor = trim($resultAnios->fields["anio_mayor"]);
	}
	else{
		$nAnioMenor = date("Y");
		$nAnioMayor = date("Y");
	...
	$sql = "  SELECT
				...
				'<a href=\"dte/view_pdf.php?sUrlPdf=' || X.path_pdf_cedible || '\">Cedible</a>' as pdf_cedible,
				...
				CASE
						WHEN D.tipo_docu = 39 OR D.tipo_docu = 41 THEN
								(SELECT trackid_xed FROM xmlenvioboleta WHERE codi_empr = X.codi_empr AND num_xed = X.num_envioboleta)
						ELSE
								(SELECT CAST(trackid_xed as VARCHAR) FROM xmlenviodte WHERE codi_empr = X.codi_empr AND num_xed = X.num_xed)
				END as track_id
			FROM
				dte_enc D,
				dte_tipo DT,
	...
	$sClassFol = ""; $sClassTD = ""; $sClassFED = ""; $sClassFVC = ""; $sClassGR = "";
	$sImgFol = ""; $sImgTD = ""; $sImgFED = ""; $sImgFVC = ""; $sImgGR = "";
	...
	elseif($_ORDER_BY_COLUM == "D.fec_venc_dte"){
		$sClassFVC = "table-active";
		$sImgFVC = $_IMG_BY_ORDER != "" ? " <img src='" . $_IMG_BY_ORDER . "' alt=''>" : "";
	}
	elseif($_ORDER_BY_COLUM == "D.giro_rec_dte"){
		$sClassGR = "table-active";
		$sImgGR = $_IMG_BY_ORDER != "" ? " <img src='" . $_IMG_BY_ORDER . "' alt=''>" : "";
	}
	...
	<div class="page-shell">
		<div class="page-hero">
			<div class="row g-3 align-items-center">
				<div class="col-lg-8"><div class="d-flex align-items-start gap-3"><div class="hero-icon"><i class="bi bi-file-earmark-text"></i></div><div><h1 class="h3 mb-2">DTE emitidos</h1><p class="mb-0 opacity-75">Consulte documentos, mantenga b&uacute;squeda, orden, paginaci&oacute;n, reenv&iacute;o y eliminaci&oacute;n legacy sin alterar el flujo operativo.</p></div></div></div>
				<div class="col-lg-4"><div class="hero-pills justify-content-lg-end"><span class="hero-pill"><i class="bi bi-funnel me-1"></i><?php echo count($filtrosResumen); ?> filtros activos</span><span class="hero-pill"><i class="bi bi-arrow-down-up me-1"></i>Orden legacy</span><span class="hero-pill"><i class="bi bi-box-arrow-up-right me-1"></i>PDF, XML y reenv&iacute;o</span></div></div>
			</div>
		</div>

		<div class="card">
			<div class="card-header d-flex flex-wrap justify-content-between gap-2 align-items-center">
				<div><div class="fw-semibold">Filtros y acciones</div><div class="small">Se conserva el formulario GET original y los mismos nombres de par&aacute;metros.</div></div>
				<div class="actions-stack"><button type="submit" form="formSearchDte" class="btn btn-light btn-sm"><i class="bi bi-search me-1"></i>Buscar</button><button type="button" class="btn btn-outline-light btn-sm" onclick="chDelEmp();"><i class="bi bi-trash me-1"></i>Eliminar selecci&oacute;n</button></div>
			</div>
			<div class="card-body">
				<form name="_FSEARCH" id="formSearchDte" method="get" action="<?php echo h($_LINK_BASE . $sLinkActual); ?>">
					<div class="row g-3 align-items-end">
						...
						<div class="col-xl-2 col-lg-3 col-md-6"><label class="form-label mb-1">Estado</label><select name="_EST_DTE" class="form-select form-select-sm"><option value="">Estado Todos</option><option value="0">DTE Cargados</option><option value="1">DTE Firmados</option><option value="3">DTE Con ERROR</option><option value="5">DTE Empaquetados</option><option value="13">DTE Enviados SII</option><option value="29">DTE Aceptados SII</option><option value="45">DTE Con Reparos SII</option><option value="77">DTE Rechazados SII</option><option value="157">DTE Enviados a Clientes</option><option value="173">DTE Con Reparos Enviados a Clientes</option><option value="413">DTE Aceptados por Clientes</option><option value="429">DTE Con Reparos Aceptados por Clientes</option></select><script>chListBoxEstado();</script></div>
				...
				<div class="filter-summary mt-3"><div class="d-flex flex-wrap align-items-center justify-content-between gap-2"><div><strong>Resumen actual</strong><div class="small text-muted">Se mantienen los filtros originales por tipo, a&ntilde;o, estado y b&uacute;squeda.</div></div><div class="text-muted small"><?php echo count($filtrosResumen); ?> filtro(s)</div></div><div class="mt-2"><?php if(count($filtrosResumen) > 0){ foreach($filtrosResumen as $filtro){ ?><span class="filter-chip"><?php echo $filtro; ?></span><?php } } else { ?><span class="text-muted small">Sin filtros adicionales. Se muestra el listado con el orden vigente.</span><?php } ?></div></div>
			</div>
		</div>

		<div class="card">
			<div class="card-header d-flex flex-wrap justify-content-between gap-2 align-items-center"><div><div class="fw-semibold">Listado de DTE</div><div class="small">Se conserva `sPagina(...)`, el formulario de borrado y las acciones PDF/XML/Re-Enviar.</div></div><div class="small">Marque filas en estados 0, 1, 3 o 77 para eliminaci&oacute;n.</div></div>
						...
						<table class="table table-sm table-hover align-middle mb-0">
							<thead>
								<tr>
									...
									<th class="<?php echo $sClassFVC; ?>"><a class="sort-link" href="<?php echo h(buildSortUrl($sLinkActual, 'D.fec_venc_dte')); ?>">Fecha Venc.</a><?php echo $sImgFVC; ?></th>
									<th>Rut Emis.</th>
									<th>Nom Emis.</th>
									<th>Giro Emis.</th>
									<th>Dir Origen</th>
									...
									<th class="<?php echo $sClassGR; ?>"><a class="sort-link" href="<?php echo h(buildSortUrl($sLinkActual, 'D.giro_rec_dte')); ?>">Giro Receptor</a><?php echo $sImgGR; ?></th>
									<th>Dir. Receptor</th>
							...
							<tbody>
							<?php if($result->EOF){ ?>
								<tr><td colspan="26"><div class="empty-state"><i class="bi bi-inbox"></i><div class="fw-semibold mt-3">No hay DTE para los filtros seleccionados</div><div class="small">Ajuste el estado, el tipo de documento, el a&ntilde;o o la b&uacute;squeda y vuelva a consultar.</div></div></td></tr>
							...
							?>
								<tr>
									...
									<td><?php echo h($dFecVenDte); ?></td>
									<td><?php echo h($nRutEmis . "-" . $sDigEmis); ?></td>
									<td><?php echo h($sNomEmis); ?></td>
									<td><?php echo h($sGiroEmis); ?></td>
									<td><?php echo h($sDirOrig); ?></td>
									<td><?php echo h($sComOrig); ?></td>
									<td><?php echo h($sCiudOrig); ?></td>
									<td><?php echo h($nRutRec . "-" . $sDigRec); ?></td>
									<td><?php echo h($sNomRec); ?></td>
									<td><?php echo h($sGiroRec); ?></td>
									<td><?php echo h($sDirRec); ?></td>
									<td><?php echo h($sComRec); ?></td>
									<td><?php echo h($sCiudRec); ?></td>
									...
									<td class="action-links"><?php if(is_file($sUrlPdf)){ ?><a href="<?php echo h($_LINK_BASE . 'dte/view_pdf.php?sUrlPdf=' . urlencode($sUrlPdf)); ?>" title="PDF">PDF</a><?php } else { ?>&nbsp;<?php } ?></td>
									<td class="action-links"><?php if(is_file($sUrlPdf)){ if($mostrarCedible){ ?><a href="<?php echo h($_LINK_BASE . 'dte/view_pdf.php?sUrlPdf=' . urlencode($sUrlPdfCedible)); ?>" title="Cedible">Ced.</a><?php } else { ?>No Aplica<?php } } else { ?>&nbsp;<?php } ?></td>
