...
// include("../include/ver_aut_adm.php");        
 ...
 function cambiarEstado($nCodCorrel, $nTipDte){
  ...
  $sLinkActual = "factura/list_recielec.php?nTipDoc=" . $nTipDoc;
...
setActiveButtonByName("clients");
	...
	function confirmCambio(nCodDoc, nTipoDte, obj){
		...
		if(confirm(_MSG_ACEPTA_RECHAZA_DTE)){
			...
			if(nTipoDte == "33" || nTipoDte == "30" || nTipoDte == "45" || nTipoDte == "46")
				enviaEstResp("factura/form_motivo.php");
			else
				F.submit();
		}			
		else
...
function enviaEstResp(URL) {
    wTipoMot=window.open(URL, "tipoMotivo","dependent=1,toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=0,resizable=1,width=400,height=400");  
...
?>            
       <table width="100%" cellspacing="0" class="list">  
        <tr>
           ...
           <th width="20%" <?php echo $sClassFecEmi; ?>>
                  <a href="javascript:location.href='<?php echo $_LINK_BASE . $sLinkActual; ?>&_ORDER_BY_COLUM=fec_emi_doc&_NIVEL_BY_ORDER=<?php echo $_NIVEL_BY_ORDER; ?>&_COLUM_SEARCH=<?php echo $_COLUM_SEARCH; ?>&_STRING_SEARCH=<?php echo $_STRING_SEARCH; ?>&_ORDER_CAMBIA=Y';">
                    Fecha Emisión</a><?php echo $sImgFecEmi; ?>
          </th>
 <!--         
          ...
          <th width="30%" <?php echo $sClassHasta; ?>>
           ...
           <th width="10%" <?php echo $sClassExen; ?>>
                  <a href="javascript:location.href='<?php echo $_LINK_BASE . $sLinkActual; ?>&_ORDER_BY_COLUM=mnt_exen_dte&_NIVEL_BY_ORDER=<?php echo $_NIVEL_BY_ORDER; ?>&_COLUM_SEARCH=<?php echo $_COLUM_SEARCH; ?>&_STRING_SEARCH=<?php echo $_STRING_SEARCH; ?>&_ORDER_CAMBIA=Y';">
                    Mnt Excento</a><?php echo $sImgExen; ?>
          </th>         
...
/********************** LISTA USUARIOS ****************************************/
        ...
        while (!$result->EOF) {
          ...
          $nTipDoc2 = trim($result->fields["tipo_docu"]);
          ...
          $strParamLink .= "&nTipDoc=" . urlencode($nTipDoc2);		  
          $strParamLink .= "&sAccion=M";      

		  $_file_pddf = $_PATH_TERCEROS_PDF . "dte-" . $nRutRec . "-" . $nDigRec . "_" . $nTipDoc2 . "_" . $nFacRef . ".pdf";
		  
			$sTemp = "";
		  if(is_file($_file_pddf))
			  $sTemp = "<a href=\"" . $_LINK_BASE . "dte/view_pdf.php?sUrlPdf=" . urlencode($_file_pddf) . "\">Ver</a>";
