	...
	function confirmCambio(nCodDoc, nTipoDte, obj){
		...
		if(confirm(_MSG_ACEPTA_RECHAZA_DTE)){
			...
			if(nTipoDte == "33" || nTipoDte == "34" || nTipoDte == "30" || nTipoDte == "45" || nTipoDte == "46")
				enviaEstResp("factura/form_motivo.php?s="+obj.value);
			else
				F.submit();
		}			
		else
...
//        $sql .= "   rut_emis_dte IN(SELECT rut_empr FROM empresa WHERE codi_empr = '" . str_replace("'","''",$_SESSION["_COD_EMP_USU_SESS"]) . "') ";
        $sql .= "     est_doc is null AND codi_empr = '" . str_replace("'","''",$_SESSION["_COD_EMP_USU_SESS"]) . "' ";
...
<?php
        if(trim($_SESSION["_COD_ROL_SESS"]) == "1")
                echo "<td><a href=\"" . $_LINK_BASE . "dte/view_setxmlrecibido.php?nFolioDte=" . $nFolio . "&nTipoDocu=" . $nTipDoc . "\">SET</a>";
?>
