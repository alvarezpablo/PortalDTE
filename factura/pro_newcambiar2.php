...
//(  include ("../include/genera_dte.php"); 
  ...
  if($_SESSION){
	...
	try{
		...
		//  $nGeneraAcuse = $_GET["nGeneraAcuse"];			// genera acuse de recibo
		//  $nAprobacion = $_GET["nAprobacion"];				// genera aceptación y rechazo de documento	
		$sRespuestaDoc =   $_GET["sRespuesta"];			// Respuesta a documento (dte)
		$sRespuestaMerca = $_GET["sRespuestaMerca"];		// respuesta a recibo de mercaderia

		$rutEmisor = $_GET["rut_rec_dte"] . "-" . $_GET["dig_rec_dte"];		// respuesta a recibo de mercaderia
		$tipoDte = $_GET["tipo_docu"];		// respuesta a recibo de mercaderia
		$folioDte = $_GET["folio_dte"];		// respuesta a recibo de mercaderia
		...
		if(trim($sRespuestaDoc) != ""){		// Genera Acuse de Recibo de Mercaderías
					/* $sRespuestaDoc
						<option value="ACD">Acepta Contenido del Documento</option>
						<option value="RCD">Reclamo al Contenido del Documento</option> */
			$_WSDL_ACEPTARECLAMA_SII = "http://cloud-ws.opendte.cl:8080/OpenDTEWS/services/IngresaAceptacionReclamo";
			$soapClient=new SoapClient($_WSDL_ACEPTARECLAMA_SII.'?wsdl');
			$soapClient->__setLocation($_WSDL_ACEPTARECLAMA_SII);
			$parametros=array();
			$parametros["rutEmisor"]=$rutEmisor;  
			$parametros["rutReceptor"]=$rutEmpr . "-" . $dvEmpr;
			$parametros["tipoDte"]=$tipoDte;
			$parametros["folioDte"]=$folioDte;
			$parametros["accionDoc"]=$sRespuestaDoc;
			$parametros["apikey"]="";
			$sjson = $soapClient->IngresaAceptacionReclamo($parametros);

			$xml = $sjson->return;
			$aXml = explode("<Codigo>", $xml);
			$aXml = explode("</Codigo>", $aXml[1]);
			$estadoAcuse = $aXml[0];
			$aXml = explode("<Glosa>", $xml);
			$aXml = explode("</Glosa>", $aXml[1]);
			$glosaAcuse = $aXml[0];

			if($estadoAcuse == "1"){
				$sql = "update dte_sii_no_openb set acuse_dte = '" . $sRespuestaDoc . "', fech_acuse_dte = now() WHERE 
				rut_prov='". $rutEmisor ."' AND tipo_docu = '$tipoDte' AND 
				folio_dte = '$folioDte' AND codi_empr = " . $_SESSION["_COD_EMP_USU_SESS"];
				$query .= $sql;
				nrExecuta($conn, $sql);

				$sql = "update documentoscompras_temp set acuse_dte = '" . $sRespuestaDoc . "', fech_acuse_dte = now()  WHERE 
				rut_rec_dte='" . $_GET["rut_rec_dte"] . "' AND tipo_docu = '$tipoDte' AND 
				fact_ref = '$folioDte' AND codi_empr = " . $_SESSION["_COD_EMP_USU_SESS"];
				$query .= $sql;
				nrExecuta($conn, $sql);
			}
		...
		if(trim($sRespuestaMerca) != ""){		// Genera Aceptación o Rechazo
				/* $sRespuestaMerca
			...
			$parametros["tipoDte"]=$tipoDte;
			$parametros["folioDte"]=$folioDte;
			$parametros["accionDoc"]=$sRespuestaMerca;
			$parametros["apikey"]="";
			$sjson = $soapClient->IngresaAceptacionReclamo($parametros);
			...
			if($estadoMerca == "1"){
				$sql = "update dte_sii_no_openb set merca_dte = '" . $sRespuestaMerca . "', fech_merca_dte = now() WHERE 
				rut_prov='". $rutEmisor ."' AND tipo_docu = '$tipoDte' AND 
				folio_dte = '$folioDte' AND codi_empr = " . $_SESSION["_COD_EMP_USU_SESS"];
				$query .= $sql;
				nrExecuta($conn, $sql);

				$sql = "update documentoscompras_temp set merca_dte = '" . $sRespuestaMerca . "', fech_merca_dte = now()  WHERE 
				rut_rec_dte='" . $_GET["rut_rec_dte"] . "' AND tipo_docu = '$tipoDte' AND 
				fact_ref = '$folioDte' AND codi_empr = " . $_SESSION["_COD_EMP_USU_SESS"];
				$query .= $sql;
				nrExecuta($conn, $sql);
		...
		$arrRespuesta = array(
			...
			"glosaAcuse" => $glosaAcuse,
			"estadoMerca" => $estadoMerca,
			"glosaMerca" => $glosaMerca
		);
// 			"query" => $query
