...
//echo $sjson;
//exit;
//echo "<br><br>JSON-RETURN : " . $sjson->return;
//exit;
	//			$sjson = json_decode(substr($sjson->return, 1), true);
				$aTemp = explode("<Glosa>",$sjson->return);
				$aTemp = explode("</Glosa>",$aTemp[1]);
$aGlosa = trim($aTemp[0]);
//$aGlosa = html_entity_decode($aGlosa);
/*$aGlosa = preg_replace_callback(
    '/"data"\s*:\s*\[\s*"((?:[^"\\\\]|\\\\.)*?)"\s*\]/s',
    function($m) {
        // escapamos los saltos de línea reales con \\n para que sea JSON válido
        $contenidoSeguro = str_replace(["\r\n", "\r", "\n"], "\\n", $m[1]);
        return '"data":["' . $contenidoSeguro . '"]';
    },
    $aGlosa
);
*/
//echo $aGlosa ;
//exit;
				$aJson = json_decode($aGlosa, true);
		$aJson['data'] = explode("\n",$aGlosa);
if ($aJson === null) {
    echo "❌ Error al decodificar JSON: " . json_last_error_msg();
    echo "<br><br><strong>Contenido original:</strong><br>";
    //echo "<pre>" . htmlspecialchars($aTemp[0]) . "</pre>";
    echo "<pre>" . $aTemp[0] . "</pre>";
    exit;
}


if (isset($aJson['data']) && is_array($aJson['data'])) {
    // Obtener la cantidad de elementos en 'data'
   $numItems = count($aJson['data']);
  echo "OAOAOAO $numItems";
...
echo $aJson["data"][$i] . "<br> .... <br>";
					...
					$razon_prov = $aLinea[4];
					$fecha_dte = $aLinea[6];
					if($aLinea[9] == "null") $aLinea[9] = "0";
					if($aLinea[10] == "null") $aLinea[10] = "0";
					if($aLinea[11] == "null") $aLinea[11] = "0";
					if($aLinea[14] == "null") $aLinea[14] = "0";
...
if (is_array($aJson) && isset($aJson["data"])) {
    echo "OK";
} else {
    // Opcional: log o manejo de error
    echo "null"; // o [] si esperas un array
}
