...
//	include("../include/ver_aut_adm.php");        
	...
	$nCodEmp = trim($_SESSION["_COD_EMP_USU_SESS"]);  
	  ...
	  $sql = "UPDATE config SET valor_config = '" . str_replace("'","''",trim($rut_usu)) . "' 
	  		  WHERE cod_config = 'RUT_AUTENTICACION_SII' AND codi_empr = '" . str_replace("'","''",trim($nCodEmp)) . "'";
	  nrExecuta($conn, $sql);
	  $sql = "UPDATE config SET valor_config = '" . str_replace("'","''",trim($dv_usu)) . "' 
	  		  WHERE cod_config = 'DV_AUTENTICACION_SII' AND codi_empr = '" . str_replace("'","''",trim($nCodEmp)) . "'";
	  nrExecuta($conn, $sql);
	  $sql = "UPDATE config SET valor_config = '" . str_replace("'","''",trim($clave_sii)) . "' 
	  		  WHERE cod_config = 'CLAVE_AUTENTICACION_SII' AND codi_empr = '" . str_replace("'","''",trim($nCodEmp)) . "'";
	  nrExecuta($conn, $sql);
...
";
  ...
  header("location:fin_user_sii.php");
  exit;    
?>
