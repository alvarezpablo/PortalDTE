...
<head>
	...
	<style>body{background:#eef2f7;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;color:#1f2937}.page-shell{max-width:820px;margin:0 auto;padding:1rem}.confirm-card{border:0;border-radius:18px;box-shadow:0 18px 40px rgba(15,23,42,.12);overflow:hidden}.confirm-body{padding:2rem 1.5rem;background:#fff}.icon-wrap{width:74px;height:74px;border-radius:22px;background:linear-gradient(135deg,#0d6efd 0%,#6ea8fe 100%);color:#fff;display:flex;align-items:center;justify-content:center;font-size:2rem}#loaderContainer{position:fixed;inset:0;background:rgba(15,23,42,.3);z-index:1050}#loaderContainerWH{vertical-align:middle;text-align:center}#loader{display:inline-block;background:#fff;border-radius:14px;padding:1rem 1.25rem;box-shadow:0 12px 28px rgba(15,23,42,.18)}</style>
...
<body onload="_body_onload();" onunload="_body_onunload();" id="mainCP">
	...
	<div class="page-shell"><div class="confirm-card card mt-4"><div class="confirm-body text-center"><div class="icon-wrap mx-auto mb-3"><i class="bi bi-person-check"></i></div><h1 class="h3 mb-2">Clientes</h1><p class="text-secondary mb-4">Los datos fueron procesados y el flujo de mantencion puede continuar sin cambios.</p><div class="alert alert-success border-0 shadow-sm mb-4"><strong>Operacion completada.</strong> La pantalla final queda alineada con ISO-8859-1 y el retorno existente.</div><div class="d-flex flex-column flex-sm-row gap-2 justify-content-center"><a href="<?php echo h($returnHref); ?>" class="btn btn-primary"><i class="bi bi-arrow-left-circle me-2"></i>Volver a Clientes</a><a href="main.php" class="btn btn-outline-secondary"><i class="bi bi-house-door me-2"></i>Ir al inicio</a></div></div></div></div>
