...
$searchOptions = array("rut_empr" => "RUT", "rs_empr" => "Razón Social", "dir_empr" => "Dirección");
...
<body onload="_body_onload();" onunload="_body_onunload();" id="mainCP">
    ...
    <div class="page-shell">
        <div class="page-hero"><div class="row g-3 align-items-center"><div class="col-lg-7"><div class="d-flex align-items-start gap-3"><div class="hero-icon"><i class="bi bi-buildings"></i></div><div><h1 class="h3 mb-2">Administración de Empresas</h1><p class="mb-0 opacity-75">Busque, edite y configure empresas del portal manteniendo la operatoria administrativa existente.</p></div></div></div><div class="col-lg-5"><div class="hero-pills justify-content-lg-end"><span class="hero-pill"><i class="bi bi-funnel me-1"></i>Filtro por RUT, razón social o dirección</span><span class="hero-pill"><i class="bi bi-grid-3x3-gap me-1"></i><?php echo $cantidadHerramientas; ?> accesos rápidos</span><span class="hero-pill"><i class="bi bi-sliders me-1"></i>Edición y configuración por empresa</span></div></div></div></div>
        <div class="card"><div class="card-header d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-2"><div><div class="fw-semibold"><i class="bi bi-lightning-charge me-2"></i>Herramientas rápidas</div><div class="small mt-1">Se mantienen los accesos administrativos del módulo.</div></div><span class="badge rounded-pill text-bg-light text-primary-emphasis">Operaciones administrativas</span></div><div class="card-body"><div class="quick-actions"><?php for($i = 0; $i < $cantidadHerramientas; $i++): ?><button type="button" class="btn btn-outline-primary" onclick="<?php echo h($aBotonEmpHerramienta['ONCLICK'][$i]); ?>"><i class="bi bi-arrow-right-circle me-1"></i><?php echo h($aBotonEmpHerramienta['SETIQUETA'][$i]); ?></button><?php endfor; ?></div></div></div>
        <div class="card"><div class="card-header d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-2"><div><div class="fw-semibold"><i class="bi bi-search me-2"></i>Búsqueda de empresas</div><div class="small mt-1">Mantiene filtros, ordenamiento y eliminación múltiple del listado original.</div></div><button type="button" class="btn btn-danger btn-sm" onclick="chDelEmp();"><i class="bi bi-trash me-1"></i>Eliminar seleccionadas</button></div><div class="card-body"><?php if($stringSearch != ""): ?><div class="filter-summary mb-4"><span class="filter-chip"><i class="bi bi-funnel-fill"></i><?php echo h($searchOptions[$columnSearch]); ?>: <?php echo h($stringSearch); ?></span></div><?php endif; ?><form name="_FSEARCH" method="get" action="<?php echo h($baseListUrl); ?>" class="row g-3 align-items-end"><div class="col-md-4 col-lg-3"><label class="form-label fw-semibold">Campo de búsqueda</label><select name="_COLUM_SEARCH" class="form-select"><option value="rut_empr" <?php echo ($columnSearch == 'rut_empr' ? 'selected' : ''); ?>>Rut (Sin DV)</option><option value="rs_empr" <?php echo ($columnSearch == 'rs_empr' ? 'selected' : ''); ?>>Razón Social</option><option value="dir_empr" <?php echo ($columnSearch == 'dir_empr' ? 'selected' : ''); ?>>Dirección</option></select></div><div class="col-md-8 col-lg-5"><label class="form-label fw-semibold">Texto a buscar</label><input type="text" name="_STRING_SEARCH" id="searchInput" class="form-control" maxlength="245" value="<?php echo h($stringSearch); ?>" placeholder="Ingrese un criterio de búsqueda"></div><div class="col-lg-4"><div class="d-flex flex-wrap gap-2 justify-content-lg-end"><button type="submit" class="btn btn-primary"><i class="bi bi-search me-1"></i>Buscar</button><a href="<?php echo h($baseListUrl); ?>" class="btn btn-outline-secondary"><i class="bi bi-x-circle me-1"></i>Limpiar</a></div></div></form></div></div>
        <div class="card"><div class="card-header d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-2"><div><div class="fw-semibold"><i class="bi bi-table me-2"></i>Listado de empresas</div><div class="small mt-1">Ordene por RUT, razón social o dirección y acceda a edición/configuración por registro.</div></div><span class="badge rounded-pill text-bg-light text-primary-emphasis">Paginación original preservada</span></div><div class="card-body p-0"><form name="_FDEL" method="post" action="empresa/pro_emp.php"><input type="hidden" name="sAccion" value="E"><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th width="20%" <?php echo $sortClasses['rut_empr']; ?>><a class="sort-link" href="<?php echo h(buildOrderUrl($baseListUrl, 'rut_empr', $nivelByOrder, $columnSearch, $stringSearch)); ?>">RUT <?php echo $sortIcons['rut_empr']; ?></a></th><th width="32%" <?php echo $sortClasses['rs_empr']; ?>><a class="sort-link" href="<?php echo h(buildOrderUrl($baseListUrl, 'rs_empr', $nivelByOrder, $columnSearch, $stringSearch)); ?>">Razón Social <?php echo $sortIcons['rs_empr']; ?></a></th><th width="28%" <?php echo $sortClasses['dir_empr']; ?>><a class="sort-link" href="<?php echo h(buildOrderUrl($baseListUrl, 'dir_empr', $nivelByOrder, $columnSearch, $stringSearch)); ?>">Dirección <?php echo $sortIcons['dir_empr']; ?></a></th><th width="16%">Acciones</th><th width="4%" class="text-center"><input type="checkbox" class="form-check-input" name="clientslistSelectAll" id="clientslistSelectAll" value="true" onclick="chDchALL();"></th></tr></thead><tbody><?php if($hayResultados): ?><?php while(!$result->EOF): ?><?php $nCodEmp = trim($result->fields["codi_empr"]); $sRutEmp = trim($result->fields["rut_empr"]) . "-" . trim($result->fields["dv_empr"]); $sRzSclEmp = trim($result->fields["rs_empr"]); $sDirEmp = trim($result->fields["dir_empr"]); $nCodAct = trim($result->fields["cod_act"]); $sGiroEmp = trim($result->fields["giro_emp"]); $sComEmp = trim($result->fields["com_emp"]); $dFecRes = trim($result->fields["fec_resolucion"]); $nResSii = trim($result->fields["num_resolucion"]); $strParamLink = "nCodEmp=" . urlencode($nCodEmp); $strParamLink .= "&sRutEmp=" . urlencode(trim($result->fields["rut_empr"])); $strParamLink .= "&sDvEmp=" . urlencode(trim($result->fields["dv_empr"])); $strParamLink .= "&sRzSclEmp=" . urlencode($sRzSclEmp); $strParamLink .= "&sDirEmp=" . urlencode($sDirEmp); $strParamLink .= "&nCodAct=" . urlencode($nCodAct); $strParamLink .= "&sGiroEmp=" . urlencode($sGiroEmp); $strParamLink .= "&sComEmp=" . urlencode($sComEmp); $strParamLink .= "&dFecRes=" . urlencode($dFecRes); $strParamLink .= "&nResSii=" . urlencode($nResSii); $strParamLink .= "&sAccion=M"; $editUrl = $_LINK_BASE . "empresa/form_emp.php?" . $strParamLink; $configUrl = $_LINK_BASE . "mantencion/list_config.php?nCodEmp=" . urlencode($nCodEmp); ?><tr><td class="fw-semibold"><?php echo h($sRutEmp); ?></td><td><a href="<?php echo h($editUrl); ?>" class="fw-semibold text-decoration-none"><?php echo h($sRzSclEmp); ?></a><div class="company-meta mt-1"><?php if($sGiroEmp != ""): ?><div>Giro: <?php echo h($sGiroEmp); ?></div><?php endif; ?><?php if($sComEmp != ""): ?><div>Comuna: <?php echo h($sComEmp); ?></div><?php endif; ?></div></td><td><div><?php echo h($sDirEmp); ?></div><div class="company-meta mt-1">Resolución SII: <?php echo h($nResSii != "" ? $nResSii : "-"); ?><?php echo ($dFecRes != "" ? " / " . h($dFecRes) : ""); ?></div></td><td><div class="actions-stack"><a href="<?php echo h($editUrl); ?>" class="btn btn-outline-primary btn-sm"><i class="bi bi-pencil-square me-1"></i>Editar</a><a href="<?php echo h($configUrl); ?>" class="btn btn-outline-secondary btn-sm"><i class="bi bi-sliders me-1"></i>Config</a></div></td><td class="text-center"><input type="checkbox" class="form-check-input" name="del[]" value="<?php echo h($nCodEmp); ?>"></td></tr><?php $result->MoveNext(); ?><?php endwhile; ?><?php else: ?><tr><td colspan="5"><div class="empty-state"><i class="bi bi-inbox"></i><h5 class="mt-3">No hay empresas para mostrar</h5><p class="mb-0">Pruebe con otro criterio de búsqueda o limpie los filtros para revisar el listado completo.</p></div></td></tr><?php endif; ?></tbody></table></div></form></div><?php if(trim($sPaginaResult) != ""): ?><div class="card-footer bg-white"><div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2"><span class="text-muted small">Paginación original preservada para mantener la navegación del módulo.</span><div class="paging"><?php echo $sPaginaResult; ?></div></div></div><?php endif; ?></div>
    </div>
</body>
</html>
