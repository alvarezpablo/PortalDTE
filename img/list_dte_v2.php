...
/*
	...
	function bajarExcel(){
		...
		if(confirm("Bajar a Excel el resultado de la busqueda?. Se descargan un máximo de 2.000 registros.")){
	...
	<tr>
		...
		<td>
			<select name="estado" size="1">
				<option value="1" selected>Cargado</option>
...
<?php
	if($_GET){
?>
<form name="_FDEL" method="post" action="">
...
//			$totalFilas = trim($resultCount->fields["t"]); 
		...
		if($totalFilas > 0){
			...
			$campos .= " ORDER BY fec_carg DESC LIMIT 20 offset " . $desde;
//			echo $campos;
	...
	?>
				...
				<?php 
					if($est_xdte < 5 and $tipo_docu != 39 && $tipo_docu != 41)
						echo "<td><input type=\"checkbox\" name=\"del[]\" value=\"<?php echo $folio_dte; ?>|<?php echo $tipo_docu; ?>\"></td>";
					else
						echo "<td>&nbsp;</td>";
				?>
					<td>
						<table class="datagrid">
							<tr>
								<td><a href="view_pdf_rem.php?c=<?php echo trim($_SESSION["_COD_EMP_USU_SESS"]); ?>&f=<?php echo $folio_dte; ?>&t=<?php echo $tipo_docu; ?>" target="_blank"><img src='../img/grp__NM__nm_icon_pdf.gif' alt='PDF'></a></td>
